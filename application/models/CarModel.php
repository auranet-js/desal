...
class CarModel extends MY_Model {
    ...
    private function _apply_car_filter($string){
        if(!empty($string)){
    ...
    public function get_car_list_with_string($string = null, $sort = null, $limit = null, $offset = 0){
        $this->_apply_car_filter($string);
        switch ($sort) {
            case 'date_asc':
            ...
            case 'default':
            default:
                // domyślnie najnowsze auta na górze (eliminuje scrollowanie na sam dół)
                $this->db->order_by('duo_cars.id', 'desc');
                break;
        }
        if($limit !== null){
    ...
    public function count_car_list_with_string($string = null){
        $this->_apply_car_filter($string);
        return $this->db->count_all_results($this->_table);
    }

    // Jedno grupowe zapytanie zamiast 2*N (N+1) — total i wartosc sprzedanych czesci per auto.
    public function get_info_bulk($car_ids){
        $info = [];
        if(empty($car_ids)){ return $info; }
        $ids = array_map('intval', $car_ids);
        $this->db->select('car_id, SUM(price) AS total, SUM(CASE WHEN sold > 0 THEN price ELSE 0 END) AS income', false);
        $this->db->where_in('car_id', $ids);
        $this->db->group_by('car_id');
        $q = $this->db->get('duo_products');
        foreach($q->result() as $r){
            $info[$r->car_id] = ['total' => $r->total, 'income' => $r->income];
        }
        return $info;
    }
    ...
    public function saveImage() {
        ...
        $targetDir = FCPATH . 'uploads/cars/' . $this->id;
        ...
        if ($this->upload->do_upload('image')) {
            ...
            $sizes = getimagesize($data['full_path']);
                        ...
                        $img->adaptive_resize($x, $y)->save($data['full_path']);
    ...
    public function getUrl($subdir = null) {
        if (!empty($this->image)) {
            ...
            return base_url('uploads/cars/' . $this->id . '/' . $subdir . $this->image);
        } else {
            return '';
        }
    }
    ...
    public function saveSketchImage($id) {
        if(empty($_FILES['image'])){ return true; }
        if ($_FILES['image']['error'] !== 0) {
            return true;
        }
...
$sizes = getimagesize($data['full_path']);
            ...
            $args = ['image' => $data['file_name'] ];
            
            return $this->update_sketch($id, $args);
        }
...
//        if($sketch->shop == 0 && $sketch->allegro == 0){
//            return 0;
//        }
